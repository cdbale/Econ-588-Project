...
# Working with the Standard Errors

With regular standard errors.
```{r}
basic_mdl <- full_data %>%
  lm(lunits ~ price + size + feature + display, data = .)

(basic_mdl_sum <- summary(basic_mdl))
```

Getting the proper coefficients.
```{r}
(basic_mdl_sum$coefficients[,1] <- (exp(basic_mdl_sum$coefficients[,1]) - 1) * 100)
```

Assuming independence but not identically distributed. (Huber-Eicker formula for standard errors).
```{r}
coeftest(basic_mdl, vcov. = vcovHC(basic_mdl, type = 'HC1'))
```

****************Assuming heteroscedasticity and autocorrelation.
```{r}
vcv <- vcovHAC.default(basic_mdl)
coeftest(basic_mdl, vcov. = vcovHAC(basic_mdl))
```

******************* Assuming not independent but identically distributed.
```{r}

```

***************** Clustering to deal with autocorrelation. At UPC level.
```{r}
coeftest(basic_mdl, vcov. = vcovHC(basic_mdl, type = 'HC1', cluster = 'upc'))
```
...
full_data <- full_data %>%
  mutate(f_upc = as_factor(upc), 
         f_store_code_uc = as_factor(store_code_uc),
         f_week_end = as_factor(as.character(week_end)),
         f_size = as_factor(size),
         f_fips_country_descr = as_factor(fips_county_descr))
```

Fixed effects model with regular standard errors.
```{r}
fe_model <- full_data %>%
  felm(lunits ~ price + feature + display | f_upc + f_store_code_uc + f_week_end, data = ., exactDOF = 'rM')
...
```{r}
(exp(fe_model$coefficients) - 1) * 100
```
...
upc_cluster_fe_model <- full_data %>%
  felm(lunits ~ price + feature + display | f_upc + f_store_code_uc + f_week_end | 0 | f_upc, 
       data = ., 
       exactDOF = 'rM')
...
store_upc_cluster_fe_model <- full_data %>%
  felm(lunits ~ price + feature + display | f_upc + f_store_code_uc + f_week_end | 0 | f_upc + f_store_code_uc, 
       data = ., 
       exactDOF = 'rM')
...
Cluster correct standard errors at the upc, store, and county level.
```{r}
county_store_upc_cluster_fe_model <- full_data %>%
  felm(lunits ~ price + feature + display | f_upc + f_store_code_uc + f_week_end | 0 | f_upc + f_store_code_uc + f_fips_country_descr,
       data = ., 
       exactDOF = 'rM')
county_store_upc_cluster_fe_sum <- summary(county_store_upc_cluster_fe_model)
county_store_upc_cluster_fe_sum
```

Store standard errors cluster corrected at the upc, store, and county level.
```{r}
county_store_upc_cluster_se <- county_store_upc_cluster_fe_sum$coefficients[,2]
```
...
se_vals <- tibble(reg_errors,
       ...
       county_store_upc_cluster_se)

reg_errors <- enframe(reg_errors, name = "Variable", value = "Normal")
robust_errors <- enframe(robust_errors, name = "Variable", value = "Robust")
upc_cluster_se <- enframe(upc_cluster_se, name = "Variable", value = "Clustered at UPC")
store_upc_cluster_se <- enframe(store_upc_cluster_se, name = "Variable", value = "Clustered at UPC and Store")
county_store_upc_cluster_se <- enframe(county_store_upc_cluster_se, name = "Variable", 
                                       value = "Clustered at UPC, Store, and County")
errors <- reg_errors %>%
  left_join(robust_errors, by = "Variable") %>%
  left_join(upc_cluster_se, by = "Variable") %>%
  left_join(store_upc_cluster_se, by = "Variable") %>%
  left_join(county_store_upc_cluster_se, by = "Variable")
